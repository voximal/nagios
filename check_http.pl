...
GetOptions(
  ...
  's'    => \$o_ssl,      'ssl'    => \$o_ssl,
  'n'    => \$o_nock,     'no-check-certificate' => \$o_nock,
...
if (@o_criticalHttpCodes or @o_criticalExpressions) {
  ...
  foreach $ccode (@o_criticalHttpCodes) {
    if ($code =~ m/$ccode/) {
      print "Status: " . $response->status_line . " on ". $url . "\n";
      exit (2);
    }
  }
  ...
  foreach $cexpr (@o_criticalExpressions) {
    if ($content =~m/$cexpr/) {
      print "Status: Critical. Matching critical regular expression on ". $url . "\n";
      exit (2);
    }
  }
} 
  ...
  if ($code >= 400) {
    print "Status: Critical (" . $response->status_line . ") on " . $url . "\n";
    exit 2;
  }
}
...
if (@o_warningHttpCodes or @o_warningExpressions) {
  ...
  # no critical condition was supplied by user
  if ($code >= 300) {
    print "Status: WARNING (" . $response->status_line . ") on ".$url."\n";
    exit 1;
  }
}
